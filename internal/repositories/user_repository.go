...
func (r *UserRepository) FetchUserByEmail(email string) (*models.User, error) {
	...
	if err := r.DB.Select("id", "name", "email").First(&user, "email = ?", email).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("user not found")
		}
		return nil, err
...
func (r *UserRepository) InsertUser(user *models.User) error {
	...
	existingUser, err := r.FetchUserByEmail(user.Email)

	// Jika tidak ada error dan user ditemukan, berarti user sudah terdaftar
